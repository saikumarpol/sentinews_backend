...
logger = logging.getLogger("sentinews.indian_market")

# Note: yfinance session removed as it was causing 401s for some users.
# We let yfinance handle its own session/cookies which is more resilient now.
...
def _get_indian_stock_price(sym: str) -> Optional[Dict]:
    ...
    # Check cache first (cache for 120 seconds)
    cached_val = _get_cached(f"stock_price_{sym}", 120)
    ...
    try:
        ...
        if val:
            res = {"last_price": val, "source": "nsepython"}
            _set_cached(f"stock_price_{sym}", res, 120)
    ...
    if q and q.get("last_price"):
        q["source"] = "yfinance"
        _set_cached(f"stock_price_{sym}", q, 120)
        return q
...
# Curated Nifty Total Market Stock Universe for Broad Movers (85 stocks across Large, Mid, Small, Micro caps)
BROAD_MARKET_SYMBOLS = [
    # Large-Caps
    "RELIANCE.NS", "TCS.NS", "INFY.NS", "HDFCBANK.NS", "ICICIBANK.NS", 
    "SBIN.NS", "ITC.NS", "BHARTIARTL.NS", "LT.NS", "M&M.NS", 
    "TATAMOTORS.NS", "TATASTEEL.NS", "NTPC.NS", "POWERGRID.NS", "SUNPHARMA.NS",
    "COALINDIA.NS", "WIPRO.NS", "AXISBANK.NS", "KOTAKBANK.NS", "HINDUNILVR.NS",
    "ASIANPAINT.NS", "MARUTI.NS", "TITAN.NS", "ULTRACEMCO.NS", "NESTLEIND.NS",
    # Mid-Caps
    "BEL.NS", "HAL.NS", "PFC.NS", "RECLTD.NS", "POLYCAB.NS",
    "VOLTAS.NS", "FEDERALBNK.NS", "TVSMOTOR.NS", "AUBANK.NS", "IDFCFIRSTB.NS",
    "BHEL.NS", "IRFC.NS", "RVNL.NS", "OBEROIRLTY.NS", "DLF.NS",
    "TATACHEM.NS", "DEEPAKNTR.NS", "PERSISTENT.NS", "COFORGE.NS", "KPITTECH.NS",
    # Small-Caps
    "CDSL.NS", "BSE.NS", "MCX.NS", "ANGELONE.NS", "HUDCO.NS",
    "IRCON.NS", "SJVN.NS", "NHPC.NS", "IREDA.NS", "RAILTEL.NS",
    "ZOMATO.NS", "YESBANK.NS", "SUZLON.NS", "NBCC.NS", "IEX.NS",
    "CUMMINSIND.NS", "MRF.NS", "EXIDEIND.NS", "GLENMARK.NS", "LAURUSLABS.NS",
    # Micro-Caps / High Growth Small Caps
    "NETWEB.NS", "KSOLVES.NS", "SIGACHI.NS", "CUPID.NS", "INDAG.NS",
    "EXCELIND.NS", "ZENSARTECH.NS", "TATAELXSI.NS", "TANLA.NS", "NEWGEN.NS",
    "MASTEK.NS", "DREAMFOLKS.NS", "TEJASNET.NS", "GENUSPOWER.NS", "RPOWER.NS",
    "SWANENERGY.NS", "GMRINFRA.NS", "SULA.NS", "YATHARTH.NS", "UTIAMC.NS"
]

def _fetch_broad_movers() -> Dict[str, List[Dict]]:
    try:
        df = yf.download(BROAD_MARKET_SYMBOLS, period="2d", interval="1d", group_by="ticker", progress=False, threads=True)
        if df.empty:
        ...
        for ticker in BROAD_MARKET_SYMBOLS:
            try:
                ticker_df = df[ticker] if isinstance(df.columns, pd.MultiIndex) else df
                ...
                prev = _safe_float(close.iloc[-2]) if len(close) > 1 else price
                change = _safe_float(price - prev) if prev is not None else 0.0
                change_pct = _safe_float((price - prev) / prev * 100) if prev and prev != 0 else 0.0

                if math.isnan(price) or math.isnan(change) or math.isnan(change_pct):
                    continue
                ...
                logger.debug("Failed processing broad mover %s: %s", ticker, e)
                continue
        ...
        logger.error("Failed fetching broad movers batch: %s", exc)
        return {"gainers": [], "losers": []}
...
def get_top_gainers():
    """
    Top 10 gainers from NSE across broad caps (Large, Mid, Small, Micro).
    """
    cached = _get_cached("broad_movers", 300)
    if cached: 
        return {"gainers": cached["gainers"], "as_of": cached["as_of"]}

    res = _fetch_broad_movers()
    res["as_of"] = datetime.now().isoformat()
    _set_cached("broad_movers", res, 300)
    return {"gainers": res["gainers"][:10], "as_of": res["as_of"]}


@router.get("/top-losers", summary="NSE top losers (live)")
def get_top_losers():
    """
    Top 10 losers from NSE across broad caps (Large, Mid, Small, Micro).
    """
    cached = _get_cached("broad_movers", 300)
    if cached: 
        return {"losers": cached["losers"], "as_of": cached["as_of"]}

    res = _fetch_broad_movers()
    res["as_of"] = datetime.now().isoformat()
    _set_cached("broad_movers", res, 300)
    return {"losers": res["losers"][:10], "as_of": res["as_of"]}
...
def get_events():
    ...
    with their resolved official company website domains.
    """
    ...
    try:
        ...
        # Limit to next 30 upcoming events
        raw_records = []
        ...
        def _resolve_one_event(rec):
            sym = rec["symbol"]
            if not sym:
                return rec
                
            if sym in _EVENT_DOMAINS_CACHE:
                rec["domain"] = _EVENT_DOMAINS_CACHE[sym]
                return rec
            ...
            try:
                ...
                if website:
                    ...
                    if netloc.startswith("www."):
                        netloc = netloc[4:]
                    _EVENT_DOMAINS_CACHE[sym] = netloc
                    rec["domain"] = netloc
                else:
                    _EVENT_DOMAINS_CACHE[sym] = None
            except Exception:
                _EVENT_DOMAINS_CACHE[sym] = None
            return rec

        with ThreadPoolExecutor(max_workers=10) as executor:
            records = list(executor.map(_resolve_one_event, raw_records))
...
async def get_stocks_in_focus(force_refresh: bool = False):
    """
