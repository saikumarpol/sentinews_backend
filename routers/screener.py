...
# Suppress yfinance extremely loud error spam so it doesn't flood logs
import logging
...
# yfinance handles its own sessions now; setting a custom requests session causes curl_cffi errors.
...
def get_screener_data():
    """
    Returns enriched screener for full NSE equity universe.
    Strategy:
      - Quick preview (~300 stocks) served on first call.
      - Background thread scans full NSE universe.
      - Each call checks if background scan has a bigger result & upgrades cache.
      - scanning=True until full dataset is available.
    """
    global _SCAN_RUNNING, _SCAN_RESULT, _SCAN_TS
